...
namespace renderer {
  ...
  inline int rgb(uint8_t r, uint8_t g, uint8_t b) { return RGB_FLAG | (r << 16) | (g << 8) | b; }
  ...
  public:
    ...
    void
    fill_rect(int x, int y, int w, int h, const std::string& ch = " ", int fg = 39, int bg = 49);
    void draw_window(
      int                x,
      int                y,
      int                w,
    ...
    void draw_block_gauge(
      ...
      int               len,
      double            percent,
      GradientDirection dir       = GradientDirection::None,
      int               start_col = -1,
      int               end_col   = -1,
      int               fg_on     = 31,
      int               fg_off    = 90
