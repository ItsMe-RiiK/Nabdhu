...
#define FMT_USE_INT128 0
  ...
  inline auto map(native_uint128) -> monostate
  {
    return {};
  }
#endif
...
#define FMT_UNICODE 1
  ...
  template <typename Char> FMT_CONSTEXPR auto compare(const Char *s1, const Char *s2, size_t n) -> int
  {
  ...
  struct is_back_insert_iterator<It,
  ...
  template <typename OutputIt> inline FMT_CONSTEXPR auto get_container(OutputIt it) -> typename OutputIt::container_type &
  {
  ...
  template <typename T, typename Enable = void> struct is_contiguous : std::false_type
  {
  };

  template <typename T>
  struct is_contiguous<T, void_t<decltype(std::declval<T &>().data()), decltype(std::declval<T &>().size()),
                                 decltype(std::declval<T &>().operator[](size_t()))>> : std::true_type
  {
  };
} // namespace detail
...
template <typename T, typename Enable = void> struct locking : std::false_type
{
};
...
public:
  ...
  template <typename Char> constexpr auto fill_unit() const -> Char
  {
    ...
    return Char(uchar(fill_data_[0]) | uchar(fill_data_[1]) << 8 | uchar(fill_data_[2]) << 16);
  }
...
public:
  ...
  template <typename Locale, FMT_ENABLE_IF(sizeof(Locale::collate) != 0)> locale_ref(const Locale &loc) : locale_(&loc)
  {
...
namespace detail
{
  ...
  template <typename Char, FMT_ENABLE_IF(is_code_unit<Char>::value)> constexpr auto to_string_view(const Char *s) -> basic_string_view<Char>
  {
    return s;
  }
...
#define FMT_TYPE_CONSTANT(Type, constant)                                                                                                  \
  ...
  template <typename T, typename Enable = std::true_type> struct is_view : std::false_type
  {
  };

  template <typename T> struct is_view<T, bool_constant<sizeof(T) != 0>> : std::is_base_of<view, T>
  {
  };
  ...
  using use_formatter =
      bool_constant<(std::is_class<T>::value || std::is_enum<T>::value || std::is_union<T>::value || std::is_array<T>::value) &&
                    !has_to_string_view<T>::value && !is_named_arg<T>::value && !use_format_as<T>::value &&
                    !use_format_as_member<U>::value>;
  ...
  public:
    constexpr explicit compile_parse_context(basic_string_view<Char> fmt, int num_args, const type *types, int next_arg_id = 0) :
  ...
  template <typename Char> FMT_CONSTEXPR auto parse_nonnegative_int(const Char *&begin, const Char *end, int error_value) noexcept -> int
  {
    ...
    return num_digits == digits10 + 1 && prev * 10ull + unsigned(p[-1] - '0') <= max ? int(value) : error_value;
  }
  ...
  public:
    ...
    FMT_CONSTEXPR explicit format_string_checker(basic_string_view<Char> fmt, arg_pack<T...>) :
        types_{ mapped_type_constant<T, Char>::value... }, named_args_{}, context_(fmt, NUM_ARGS, types_),
  ...
  public:
    ...
    template <typename U> FMT_CONSTEXPR20 void append(const U *begin, const U *end)
    {
    ...
    template <typename Idx> constexpr auto operator[](Idx index) const -> const T &
    {
      return ptr_[index];
    }
  };
  ...
  template <typename OutputIt, typename InputIt, typename = void> struct has_append : std::false_type
  {
  };
  ...
  template <typename OutputIt, typename T, typename = void> struct has_insert : std::false_type
  {
  };

  template <typename OutputIt, typename T>
  struct has_insert<OutputIt, T, void_t<decltype(get_container(std::declval<OutputIt>()).insert({}, std::declval<T>(), std::declval<T>()))>>
      : std::true_type
  {
  };
  ...
  template <typename T, typename InputIt, typename OutputIt,
            FMT_ENABLE_IF(is_back_insert_iterator<OutputIt>() && has_append<OutputIt, InputIt>())>
  ...
  template <typename T, typename InputIt, typename OutputIt,
            FMT_ENABLE_IF(is_back_insert_iterator<OutputIt>() && !has_append<OutputIt, InputIt>() && has_insert<OutputIt, InputIt>())>
  FMT_CONSTEXPR auto copy(InputIt begin, InputIt end, OutputIt out) -> OutputIt
  {
...
#define FMT_BUILTIN
  ...
  public:
    ...
    constexpr FMT_INLINE value(unsigned long x FMT_BUILTIN) : value(ulong_type(x)) {}
    ...
    template <typename T, FMT_ENABLE_IF(is_code_unit<T>::value)> constexpr FMT_INLINE value(T x FMT_BUILTIN) : char_value(x)
    {
    ...
    constexpr FMT_INLINE value(volatile void *x FMT_BUILTIN) : pointer(const_cast<const void *>(x)) {}

    constexpr FMT_INLINE value(const volatile void *x FMT_BUILTIN) : pointer(const_cast<const void *>(x)) {}
    ...
    template <typename T, FMT_ENABLE_IF(use_format_as<T>::value)> constexpr value(const T &x) : value(format_as(x)) {}

    template <typename T, FMT_ENABLE_IF(use_format_as_member<T>::value)> constexpr value(const T &x) : value(formatter<T>::format_as(x)) {}

    template <typename T, FMT_ENABLE_IF(is_named_arg<T>::value)> constexpr value(const T &named_arg) : value(named_arg.value) {}
  ...
  private:
    ...
    template <typename T> static void format_custom(void *arg, parse_context<char_type> &parse_ctx, Context &ctx)
    {
  ...
  template <typename Context, typename First, typename... T> constexpr auto encode_types() -> ullong
  {
    return unsigned(stored_type_constant<First, Context>::value) | (encode_types<Context, T...>() << packed_arg_bits);
  }

  template <typename Context, typename... T, size_t NUM_ARGS = sizeof...(T)> constexpr auto make_descriptor() -> ullong
  {
  ...
  template <typename Context, int NUM_ARGS, int NUM_NAMED_ARGS, ullong DESC> struct named_arg_store
  {
  ...
  template <typename Context, int NUM_ARGS, int NUM_NAMED_ARGS, ullong DESC> struct format_arg_store
  {
  ...
  template <bool B> constexpr bool enforce_compile_checks()
  {
    ...
    static_assert(FMT_USE_CONSTEVAL && B, "FMT_ENFORCE_COMPILE_STRING requires format strings to use FMT_STRING");
#endif
    return true;
  }
  ...
  template <typename T1, typename T2, typename... Tail> constexpr auto is_locking() -> bool
  {
...
template <typename T, typename Char = char> using named_arg = detail::named_arg<T, Char>;
...
private:
  ...
  template <int NUM_ARGS, int NUM_NAMED_ARGS, ullong DESC> using store = detail::format_arg_store<Context, NUM_ARGS, NUM_NAMED_ARGS, DESC>;
...
public:
  ...
  template <int NUM_ARGS, int NUM_NAMED_ARGS, ullong DESC, FMT_ENABLE_IF(NUM_ARGS <= detail::max_packed_args)>
  ...
  {
    return int(is_packed() ? ullong(detail::max_packed_args) : desc_ & ~detail::is_unpacked_bit);
  }
};
...
public:
  ...
  constexpr context(iterator out, format_args args, locale_ref loc = {}) : out_(out), args_(args), loc_(loc) {}
...
private:
  ...
  using checker = detail::format_string_checker<char, int(sizeof...(T)), num_static_named_args,
...
template <typename Context = context, typename... T, int NUM_ARGS = int(sizeof...(T)),
          int NUM_NAMED_ARGS = detail::count_named_args<T...>(), ullong DESC = detail::make_descriptor<Context, T...>()>
...
template <typename... T>
using vargs =
    detail::format_arg_store<context, int(sizeof...(T)), detail::count_named_args<T...>(), detail::make_descriptor<context, T...>()>;
 ...
 */
template <typename T> inline auto arg(const char *name, const T &arg) -> named_arg<T>
{
  return { name, arg };
}
...
template <size_t N> FMT_DEPRECATED auto vformat_to(char (&out)[N], string_view fmt, format_args args) -> format_to_result
{
...
template <typename... T> FMT_INLINE void print(format_string<T...> fmt, T &&...args)
{
  ...
  detail::is_locking<T...>() ? vprint_buffered(stdout, fmt.str, va) : vprint(fmt.str, va);
}
...
template <typename... T> FMT_INLINE void print(FILE *f, format_string<T...> fmt, T &&...args)
{
  ...
  detail::is_locking<T...>() ? vprint_buffered(f, fmt.str, va) : vprint(f, fmt.str, va);
}
