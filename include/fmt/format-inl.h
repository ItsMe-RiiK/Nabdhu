...
namespace detail
{
  ...
  namespace dragonbox
  {
...
#else
        static constexpr uint64_t powers_of_5_64[] = { 0x0000000000000001, 0x0000000000000005, 0x0000000000000019, 0x000000000000007d,
    ...
    template <typename T> FMT_INLINE auto shorter_interval_case(int exponent) noexcept -> decimal_fp<T>
    {
...
#define FMT_USE_FLOCKFILE 1
  ...
  template <typename F> struct has_flockfile<F, void_t<decltype(flockfile(&std::declval<F &>()))>> : bool_constant<FMT_USE_FLOCKFILE != 0>
  {
  };
