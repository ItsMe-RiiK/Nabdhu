...
long ProcessManager::get_system_uptime() { return static_cast<long>(get_uptime_internal()); }

long ProcessManager::get_hertz() const { return hertz; }
...
std::vector<double> ProcessManager::get_core_cpu_usage() { return last_core_usages; }
  ...
  while (fgets(buffer, sizeof(buffer), fp) != NULL) {
    ...
    if (line == "Memory Device") {
      ...
      if (line.find("Size:") != std::string::npos) {
        if (
          line.find("No Module Installed") == std::string::npos
        ...
        line.find("Type:") != std::string::npos && device_has_size
        ...
        line.find("Speed:") != std::string::npos && device_has_size
      ...
      if (is_pid) {
          ...
          if (fd_cmd != -1) {
            ...
            if (bytes_read > 0) {
              ...
              if (!cmdline.empty()) {
                ...
                if (
                  cmd_name == "exe" || cmd_name == "AppRun" || cmd_name == "chrome_crashpad_handler"
                  || info.name == "Main" || info.name == "Worker" || info.name == "Zygote"
                ) {
                ...
                if (
                  info.name == "exe" || info.name == "Main" || info.name == "Worker"
                  || cmd_name.rfind(info.name, 0) == 0
                ) {
                  info.name = cmd_name;
                }
              }
        ...
        if (fd_stat != -1) {
          ...
          if (bytes_read > 0) {
            ...
            if (rp != nullptr) {
              if (
                info.name.empty() || info.name == "self" || info.name == "Self"
                || info.name == "exe"
              ) {
              ...
              if (
                ...
                if (!info.command.empty()) {
                  ...
                  std::string first_cmd = (space_pos != std::string::npos)
                                          ? info.command.substr(0, space_pos)
                                          : info.command;
              ...
              unsigned long long lldummy;
              long               rss   = 0;
              int                items = sscanf(
                rest,
                "%c %d %d %d %d %d %lu %lu %lu %lu %lu %llu "
                "%llu %lu %lu %lu %lu %d %lu %llu %lu %ld",
                &state_char, &info.ppid, &dummy, &dummy, &dummy, &dummy, &ldummy, &ldummy, &ldummy,
                &ldummy, &ldummy, &utime, &stime, &ldummy, &ldummy, &ldummy, &ldummy, &info.threads,
                &ldummy, &lldummy, &ldummy, &rss
              );
                ...
                if (!first_process_run) {
                  ...
                  if (prev_it != prev_process_cpu.end()) {
                    ...
                    if (uptime_diff > 0) {
                      ...
                      info.cpu_usage = (100.0 * ((double) total_time_diff / hertz) / uptime_diff)
  ...
  if (pipe) {
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
      ...
      auto        get_next = [&]() {
        ...
        std::string val =
          (next == std::string::npos) ? line.substr(pos) : line.substr(pos, next - pos);
                ...
                if (fp) {
                  ...
                  if (fgets(buf, sizeof(buf), fp) != nullptr) {
                    ...
                    if (pos != std::string::npos) {
                      ...
                      while (
                        !cached_intel_name.empty()
                        && (cached_intel_name.back() == '\n' || cached_intel_name.back() == '\r'))
              ...
              if (fp) {
                ...
                if (fgets(buf, sizeof(buf), fp) != nullptr) {
                  ...
                  if (pos != std::string::npos) {
                    ...
                    while (!cached_amd_name.empty()
                           && (cached_amd_name.back() == '\n' || cached_amd_name.back() == '\r'))
