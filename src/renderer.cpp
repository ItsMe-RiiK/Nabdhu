    ...
    for (int cy = 0; cy < h; ++cy) {
        for (int cx = 0; cx < w; ++cx) {
            ...
            for (int dy = 0; dy < 4; ++dy) {
                ...
                if (pixel_y_from_bottom < h_left) left_val++;
                if (pixel_y_from_bottom < h_right) right_val++;
            }
        ...
        for (int x = 0; x < width; ++x) {
            ...
            if (back != front) {
                ...
                if (back.fg != last_fg || back.bg != last_bg || back.bold != last_bold || back.inverted != last_inv) {
                    ...
                    if (back.bold) fmt::format_to(std::back_inserter(out), ";1");
                    if (back.inverted) fmt::format_to(std::back_inserter(out), ";7");
