...
namespace renderer {
  ...
  void Renderer::draw_text(
    int x, int y, const std::string& text, int fg, int bg, bool bold, bool inverted, int max_w
  )
  {
    if (y < 0 || y >= height)
      return;
  ...
  void Renderer::draw_window(
    int x, int y, int w, int h, const std::string& title, int title_color, int border_color, int bg
  )
  {
    if (w <= 0 || h <= 0)
      return;
      ...
      if (cur_x >= 0 && cur_x < width) {
        ...
        if (i < filled) {
          if (dir != GradientDirection::None && start_col != -1) {
            ...
            if (
              dir == GradientDirection::ColorToColor
              && ((start_col == 32 && end_col == 31) || (start_col == 31 && end_col == 32))
            ) {
              if (t <= 0.5f) {
    ...
    for (int y = 0; y < height; ++y) {
      for (int x = 0; x < width; ++x) {
        ...
        if (back != front) {
          ...
          if (
            back.fg != last_fg || back.bg != last_bg || back.bold != last_bold
            || back.inverted != last_inv
          ) {
            ...
            if (back.fg & RGB_FLAG) {
              fmt::format_to(
                std::back_inserter(out), ";38;2;{};{};{}", (back.fg >> 16) & 0xFF,
                (back.fg >> 8) & 0xFF, back.fg & 0xFF
              );
            }
            else {
            ...
            if (back.bg & RGB_FLAG) {
              fmt::format_to(
                std::back_inserter(out), ";48;2;{};{};{}", (back.bg >> 16) & 0xFF,
                (back.bg >> 8) & 0xFF, back.bg & 0xFF
              );
            }
            else {
