...
void UIManager::refresh_data()
{
  ...
  mem_usage_str =
      "Memory: " + format_bytes((global_mem.total_kb - global_mem.available_kb) * 1024) + " / " + format_bytes(global_mem.total_kb * 1024);
  ...
  net_usage_str =
      "Network (Rx: " + std::to_string((int)net_rx_history.back()) + "kbps, Tx: " + std::to_string((int)net_tx_history.back()) + "kbps)";
}
...
void UIManager::apply_filter()
{
  ...
  if (current_sort == SortBy::CPU)
  {
    ...
    for (size_t i = 0, offset = 0; i < filtered_procs.size(); i++)
    {
      ...
      {
        std::rotate(filtered_procs.begin() + offset, filtered_procs.begin() + i, filtered_procs.begin() + i + 1);
        offset++;
      }
    }
  }
...
void UIManager::run(int argc, char *argv[])
{
  ...
  while (running)
  {
    ...
    auto elapsed_data = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_data_refresh).count();
    auto elapsed_clock = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_clock_tick).count();
    ...
    else if (dirty)
    {
      ...
      if (time_till_draw < 0)
        time_till_draw = 0;
      if (time_till_draw < event_timeout)
