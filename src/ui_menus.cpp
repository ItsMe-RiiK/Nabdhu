...
void UIManager::perform_action()
{
  ...
  std::vector<std::string> ctx = (tab_selected == 0) ? std::vector<std::string>{"End Task", "Open Location", "Cancel"} :
  ...
  if (tab_selected == 0 && proc_selected >= 0 && proc_selected < (int)filtered_procs.size())
  {
    ...
    {
      std::string cmd = "xdg-open $(dirname $(readlink -f /proc/" + std::to_string(pid) + "/exe)) 2>/dev/null &";
      system(cmd.c_str());
    }
  }
  else if (tab_selected == 1 && svc_selected >= 0 && svc_selected < (int)filtered_svcs.size())
  {
    ...
    {
      std::string cmd = "xdg-open $(dirname $(systemctl show -p FragmentPath " + sname + " | cut -d= -f2)) 2>/dev/null &";
      system(cmd.c_str());
    }
  }
}