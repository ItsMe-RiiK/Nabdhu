...
void UIManager::handle_input()
{
  ...
  bool is_held =
    (ev.key == last_key && ev.ch == last_ch
     && std::chrono::duration_cast<std::chrono::milliseconds>(now - last_key_time).count() < 600);
  ...
  if (is_held) {
    bool allow_hold =
      (ev.key == input::KeyCode::Up || ev.key == input::KeyCode::Down
       || ev.key == input::KeyCode::Right || ev.key == input::KeyCode::Left
       || ev.key == input::KeyCode::MouseScrollUp || ev.key == input::KeyCode::MouseScrollDown
       || (in_search_mode && ev.key == input::KeyCode::Backspace)
       || (in_search_mode && ev.key == input::KeyCode::Char));
