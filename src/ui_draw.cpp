...
void UIManager::draw()
{
  ...
  if (w < 80 || h < 25) {
    if (w >= 33 && h >= 4) {
      ...
      render.draw_text(
        w / 2 - 11, box_y + 2, fmt::format("Size: {}x{} (Min 80x25)", w, h), 33, 49, true
      );
    }
    else {
  ...
  if (battery_info.present) {
    ...
    render.draw_block_gauge(
      pos[2] + 5, 0, 10, battery_info.capacity / 100.0, renderer::GradientDirection::ColorToColor,
      31, 32, bat_col, 90
    );
  ...
  if (show_cpu) {
    ...
    if (cpu_h > 2) {
      ...
      std::string fluff[]    = {"Intel(R)",     "Core(TM)",
                                "CPU",          "Processor",
                                "14th Gen",     "13th Gen",
                                "12th Gen",     "11th Gen",
                                "10th Gen",     "9th Gen",
                                "8th Gen",      "7th Gen",
                                "AMD ",         "with Radeon Graphics",
      ...
      render.draw_text(
        2, y_cursor + 1, final_name + " @ " + cpu_hw.speed, 37, 49, false, false, left_w - 3
      );
      ...
      render.draw_block_gauge(
        graph_x, y_cursor + 2, graph_len, total_pct / 100.0,
        renderer::GradientDirection::ColorToColor, 32, 31
      );
    ...
    if (cpu_h > 4) {
      ...
      for (size_t i = 0; i < core_usages.size(); i++) {
        ...
        std::string temp_str =
          (temp_val > 0) ? (std::to_string(temp_val) + renderer::Symbols::degree + "C") : "";
        int temp_col = get_temp_color(temp_val);

        std::string core_name = "C" + std::to_string(i);
        render.draw_text(cx, cy, core_name, 36);
        render.draw_block_gauge(
          cx + 3, cy, col_w - 8, core_usages[i] / 100.0, renderer::GradientDirection::ColorToColor,
          32, 31
        );
        if (!temp_str.empty())
      ...
      if (!gpu_infos.empty() && cpu_h > cores_h + 4) {
        ...
        render.draw_block_gauge(
          graph_x, gpu_y, graph_len, gpu.utilization / 100.0,
          renderer::GradientDirection::ColorToColor, 32, 31
        );
  ...
  if (show_mid) {
    ...
    if (show_mem) {
      ...
      if (mid_h > 4) {
        ...
        if (m_w > 4)
          render.draw_block_gauge(
            2, y_cursor + 3, m_w - 4, used / total, renderer::GradientDirection::DarkToLight, 31,
            -1, 31
          );
        ...
        if (mid_h > 6) {
          ...
          if (m_w > 4)
            render.draw_block_gauge(
              2, y_cursor + 5, m_w - 4, cached / total, renderer::GradientDirection::DarkToLight,
              36, -1, 36
            );
        }
        if (mid_h > 8) {
          ...
          if (m_w > 4)
            render.draw_block_gauge(
              2, y_cursor + 7, m_w - 4, free / total, renderer::GradientDirection::DarkToLight, 32,
              -1, 32
            );
        }
      }
      ...
      if (mid_h > 2) {
        ...
        for (int i = 0; i < d_list; i++) {
          ...
          if (d_w > 25) {
            ...
            render.draw_block_gauge(
              d_x + 15, dy + 1, d_w - 17, used_pct, renderer::GradientDirection::DarkToLight, 31,
              -1, 31
            );
          }
          else {
            ...
            if (d_w > 12)
              render.draw_block_gauge(
                d_x + 10, dy + 1, d_w - 12, used_pct, renderer::GradientDirection::DarkToLight, 31,
                -1, 31
              );
          }

          double free_pct =
            (double) (disks[i].total_bytes - disks[i].used_bytes) / disks[i].total_bytes;
          if (d_w > 25) {
            render.draw_text(d_x + 2, dy + 2, fmt::format("Free: {:.1f}%", free_pct * 100.0), 37);
            render.draw_block_gauge(
              d_x + 15, dy + 2, d_w - 17, free_pct, renderer::GradientDirection::DarkToLight, 32,
              -1, 32
            );
          }
          else {
            ...
            if (d_w > 12)
              render.draw_block_gauge(
                d_x + 10, dy + 2, d_w - 12, free_pct, renderer::GradientDirection::DarkToLight, 32,
                -1, 32
              );
          }
        }
    ...
    if (net_h > 2) {
      ...
      if (net_h > 4) {
        ...
        for (const auto& net : networks) {
          if (net.name != "lo") {  // exclude loopback
        ...
        std::string tbuf = fmt::format(
          "Totl: {:6.2f}{} | {:8.2f}{}   ", rx_p.first, rx_p.second, tx_p.first, tx_p.second
        );
  ...
  if (show_proc) {
    ...
    if (tab_selected == 0) {
      ...
      std::string header = fmt::format(
        " {:<7}{}{:<{}.{}}{}{:<{}.{}}{}{:<7}{}{:<7}{}{:>6}", "PID", pad, "Program", prog_len,
        prog_len, pad, "Status", status_len, status_len, pad, "User", pad, "Memory", pad, "CPU%"
      );
      ...
      for (int i = 0; i < list_h && i + proc_scroll < (int) filtered_procs.size(); i++) {
        ...
        std::string formatted = fmt::format(
          " {:<7d}{}{:<{}.{}}{}{:<{}.{}}{}{:<7.7}{}{:<7.7}{}{:5.1f}%", p->pid, pad, p->name,
          prog_len, prog_len, pad, p->state, status_len, status_len, pad, p->user, pad,
          format_bytes(p->memory_kb * 1024), pad, p->cpu_usage
        );
        ...
        if (idx == proc_selected) {
          render.draw_text(
            list_x + 2, list_start_y + i, formatted, 30, 47, false, false, target_len
          );
        }
        else {
          render.draw_text(
            list_x + 2, list_start_y + i, formatted, 37, 49, false, false, target_len
          );
        }
      }
      ...
      std::string header = fmt::format(
        " {:<{}.{}}   {:<{}.{}}   {:<{}.{}}", "Service Name", name_len, name_len, "Status",
        status_len, status_len, "Description", desc_len, desc_len
      );
      if ((int) header.length() < target_len)
        header.append(target_len - header.length(), ' ');
      if (avail_h > 3)
      ...
      for (int i = 0; i < list_h && i + svc_scroll < (int) filtered_svcs.size(); i++) {
        ...
        std::string formatted = fmt::format(
          " {:<{}.{}}   {:<{}.{}}   {:<{}.{}}", s->name, name_len, name_len, s->active_state,
          status_len, status_len, s->description, desc_len, desc_len
        );
        if ((int) formatted.length() < target_len)
          formatted.append(target_len - formatted.length(), ' ');
  ...
  if (show_main_menu) {
    ...
    for (int i = 0; i < 3; i++) {
      if (main_menu_selected == i)
        render.draw_text(
          mx + mw / 2 - options[i].length() / 2, my + 5 + i * 2, options[i], 30, 47, true
        );
      else
        render.draw_text(
          mx + mw / 2 - options[i].length() / 2, my + 5 + i * 2, options[i], 37, 49, false
        );
    }
  }
